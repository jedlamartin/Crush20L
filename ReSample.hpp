...
public:
    ...
    float& operator[](int i) { return kaiser[i < 0 ? -i : i]; }

    size_t size() const { return this->kaiser.size(); }
    ...
    float besselI0(float x) {
        ...
        while(term > epsilon * sum) {
            ...
            term = std::pow(x / 2.0f, 2 * k) /
                   (factorial * factorial);    // Jelenlegi tag
            sum += term;
            k += 1.0f;
        }
...
public:
    ...
    float& operator[](int i) { return sinc[i < 0 ? -i : i]; }
    ...
    void configure(float sampleRate) {
        ...
        for(int i = 0; i <= N; i++) {
            for(int delta = 0; delta < iSize; delta++) {
                float index =
                    static_cast<float>(i) + (1.0f / static_cast<float>(iSize)) *
                                                static_cast<float>(delta);
                (*this)(i, delta) =
                    std::sin(2.0f * 3.141592f * fc * index * T) /
                    (2.0f * 3.141592f * fc * index * T);
            }
        }
        (*this)[0] = 1;
    }

    size_t size() const { return this->sinc.size(); }
...
private:
    ...
    std::vector<std::vector<float>> interpolatedBuf;
...
public:
    ...
    void interpolate(std::vector<std::vector<float>>& buffer) {
        int channelSize = buffer.size();
        int originalBufSize = buffer[0].size();
        ...
        std::vector<std::vector<float>> x(
            channelSize, std::vector<float>(originalBufSize + N, 0.0f));
        for(int channel = 0; channel < channelSize; channel++) {
            // x.copyFrom(channel, N, buffer, channel, 0, originalBufSize);
            for(int i = 0; i < originalBufSize; i++) {
                x[channel][N + i] = buffer[channel][i];
            }

            std::vector<float>& currentSample = x[channel];
            for(int i = 0; i < N; i++) {
        ...
        // interpolatedBuf.setSize(channelSize, interpolatedBufSize,
        // true, true, true);
        for(int i = 0; i < interpolatedBuf.size(); i++) {
            interpolatedBuf[i].resize(interpolatedBufSize, 0.0f);
        }
        interpolatedBuf.resize(channelSize,
                               std::vector<float>(interpolatedBufSize, 0.0f));
        for(int channel = 0; channel < channelSize; channel++) {
            const std::vector<float>& channelSamples = x[channel];
            std::vector<float>& iSamples = interpolatedBuf[channel];
            // bevart mintak
            // az elsot automatikusan kitoltjuk, hogy a buffereles jo legyen
            iSamples[0] = channelSamples[0];
    ...
    void decimate(std::vector<std::vector<float>>& buffer) {
        int channelSize = buffer.size();
        int originalBufSize = buffer[0].size();
        int interpolatedBufSize = this->interpolatedBuf[0].size();
        ...
        std::vector<std::vector<float>> x(
            channelSize,
            std::vector<float>(interpolatedBufSize + N * iSize, 0.0f));

        for(int channel = 0; channel < channelSize; channel++) {
            // x.copyFrom(channel, N * iSize,
            // interpolatedBuf, channel, 0,
            // interpolatedBufSize);
            for(int i = 0; i < interpolatedBufSize; i++) {
                x[channel][N * iSize + i] = interpolatedBuf[channel][i];
            }
            std::vector<float>& currentSample = x[channel];
        ...
        for(int channel = 0; channel < channelSize; channel++) {
            const std::vector<float>& iSamples = x[channel];
            std::vector<float>& samples = buffer[channel];
    ...
    void process(
        std::function<void(std::vector<std::vector<float>>&)> processBlock) {
