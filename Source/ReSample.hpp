    ...
    void configure(float sampleRate) {
        ...
        for(int i = 0; i <= N; i++) {
            for(int delta = 0; delta < iSize; delta++) {
                ...
                (*this)(i, delta) =
                    std::sin(2.0f * juce::MathConstants<float>::pi * fc *
                             index * T) /
                    (2.0f * juce::MathConstants<float>::pi * fc * index * T);
            }
        }
        (*this)[0] = 1;
    }
...
public:
    ...
    void interpolate(juce::AudioBuffer<float>& buffer) {
        ...
        interpolatedBuf.setSize(
            channelSize, interpolatedBufSize, true, true, true);
        for(int channel = 0; channel < channelSize; channel++) {
    ...
    void decimate(juce::AudioBuffer<float>& buffer) {
        ...
        juce::AudioBuffer<float> x(channelSize,
                                   interpolatedBufSize + N * iSize);
        for(int channel = 0; channel < channelSize; channel++) {
            x.copyFrom(channel,
                       N * iSize,
                       interpolatedBuf,
