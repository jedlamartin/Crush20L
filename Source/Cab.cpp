...
Cabs::Cabs() :
    default_cab {}, loaded_cab {}, ir_loading_buffer {},
    activeCab {ActiveCab::DEFAULT_CAB}, cab_buf {} {
    loadWavFromMemory(BinaryData::orange_cab_wav,
                      static_cast<size_t>(BinaryData::orange_cab_wavSize),
                      default_cab);
}

void Cabs::loadCab() {
    loadWavFromFileChooser(ir_loading_buffer, new_cab_ready);
}

void Cabs::processBlock(juce::AudioBuffer<float>& buffer, ActiveCab ac) {
    if(new_cab_ready.load()) {
        std::copy(ir_loading_buffer.begin(),
                  ir_loading_buffer.end(),
                  loaded_cab.begin());
    ...
    else if(ac == LOADED_CAB)
        activeCabArray = &loaded_cab;
    else
        return;

    const auto& cabIr = *activeCabArray;

    const int channelNumber = buffer.getNumChannels();
    cab_buf.resize(channelNumber, CircularBuffer<float, FIR_TAPS> {0.0f});
