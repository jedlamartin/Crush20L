...
OrangeCrush20LAudioProcessor::OrangeCrush20LAudioProcessor()
    ...
    parameters(*this,
               nullptr,
               juce::Identifier("OrangeCrush20L"),
               this->createParameterLayout()),
    stage2Attachment(&stage2, this), stage3Attachment(&stage3, this),
    stage4Attachment(&stage4, this), stage5Attachment(&stage5, this), cabs {} {
    // Stage2
    ...
    // Stage3
    this->stage3.initParameters(parameters.getRawParameterValue("od"),
                                parameters.getRawParameterValue("odButton"));
    this->parameters.addParameterListener("od", &stage3Attachment);
    this->parameters.addParameterListener("odButton", &stage3Attachment);

    // Stage4
    this->stage4.initParameters(parameters.getRawParameterValue("bass"),
                                parameters.getRawParameterValue("mid"),
                                parameters.getRawParameterValue("treble"));
...
void OrangeCrush20LAudioProcessor::changeProgramName(
    int index,
    const juce::String& newName) {}

//==============================================================================
void OrangeCrush20LAudioProcessor::prepareToPlay(double sampleRate,
                                                 int samplesPerBlock) {
...
void OrangeCrush20LAudioProcessor::processBlock(
    ...
    if(this->parameters.getRawParameterValue("power")->load() >= 0.5f) {
        float inputGain =
            std::pow(10,
                     parameters.getRawParameterValue("input")->load() / 20.0f) *
            3.0f;
        ...
        // Cab IR
        const int currentCabChoice = static_cast<int>(
            parameters.getRawParameterValue("cabButton")->load());
        ...
        float outputGain =
            std::pow(
                10, parameters.getRawParameterValue("output")->load() / 20.0f) *
            0.04f;
...
juce::AudioProcessorValueTreeState::ParameterLayout
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout vtsParameterLayout {
        ...
        std::make_unique<juce::AudioParameterFloat>(
            "gain",
            "Gain",
            juce::NormalisableRange<float>(0.01f, 10000.0f, 0.05f, 0.3f),
            0.01f),

        std::make_unique<juce::AudioParameterFloat>(
            "bass",
            "Bass",
            juce::NormalisableRange<float>(0.01f, 250000.0f, 0.5f, 0.3f),
            25000.0f),

        std::make_unique<juce::AudioParameterFloat>(
            "mid",
            "Mid",
            juce::NormalisableRange<float>(0.01f, 25000.0f, 0.5f, 0.3f),
            2500.000f),
        ...
        std::make_unique<juce::AudioParameterFloat>(
            "od",
            "Overdrive",
            juce::NormalisableRange<float>(0.01f, 50000.0f, 0.5f, 6.58f),
            0.01f),

        std::make_unique<juce::AudioParameterBool>(
            "odButton", "Overdrive Button", false),
        std::make_unique<juce::AudioParameterFloat>(
            "vol",
            "Volume",
            juce::NormalisableRange<float>(0.01f, 50000.0f, 0.5f, 0.3f),
            0.01f),
        ...
        std::make_unique<juce::AudioParameterChoice>(
            "cabButton",
            "Cabinet Selector",
            juce::StringArray {"Default Cab", "No Cab", "Import"},
            0)};
...
void OrangeCrush20LAudioProcessor::loadCab() { cabs.loadCab(); }
