...
void Stage6::processBlock(std::vector<std::vector<float>>& buffer) {
  ...
  for (int channel = 0; channel < channelNumber; channel++) {
    ...
    for (int i = 0; i < channelSamples.size(); i++) {
      ...
      channelSamples[i] =
          A * yBuffer[channel] + B * channelSamples[i] + C * uBuffer[channel];

      // Cutoff
      if (channelSamples[i] > this->cutOffVoltage) {
        channelSamples[i] = this->cutOffVoltage;
        tmp = (this->cutOffVoltage - A * yBuffer[channel] -
               C * uBuffer[channel]) /
              B;
