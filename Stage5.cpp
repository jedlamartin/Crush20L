...
void Stage5::processBlock(std::vector<std::vector<float>>& buffer) {
    ...
    for(int channel = 0; channel < channelNumber; channel++) {
        ...
        for(int i = 0; i < channelSamples.size(); i++) {
            float yCurrent =
                params.A * yBuffer[channel][0] +
                params.B * yBuffer[channel][1] + params.C * channelSamples[i] +
                params.D * uBuffer[channel][0] + params.E * uBuffer[channel][1];
            float yUpdate = updatedParams.A * yUBuffer[channel][0] +
                            updatedParams.B * yUBuffer[channel][1] +
                            updatedParams.C * channelSamples[i] +
                            updatedParams.D * uUBuffer[channel][0] +
                            updatedParams.E * uUBuffer[channel][1];
            ...
            float tmp = yCurrent;
            float tmpU = yUpdate;
            yCurrent = params.F * yNegBuffer[channel] + params.G * yCurrent +
                       params.H * uNegBuffer[channel];
            yUpdate = updatedParams.F * yUNegBuffer[channel] +
                      updatedParams.G * yUpdate +
                      updatedParams.H * uUNegBuffer[channel];
            ...
            tmp = yCurrent;
            tmpU = yUpdate;
            yCurrent = params.I * yVolBuffer[channel][0] +
                       params.J * yVolBuffer[channel][1] + params.K * yCurrent +
                       params.L * uVolBuffer[channel][0] +
                       params.M * uVolBuffer[channel][1];
            yUpdate = updatedParams.I * yUVolBuffer[channel][0] +
                      updatedParams.J * yUVolBuffer[channel][1] +
                      updatedParams.K * yUpdate +
                      updatedParams.L * uUVolBuffer[channel][0] +
                      updatedParams.M * uUVolBuffer[channel][1];
...
void Stage5::configure(double sampleRate) {
    ...
    float G = 1 / (184467.44073709551616000000 * T * T +
                   8116.93632731367731603232 * T + 0.01623313478486440625);
    newValues.A =
        -(368934.88147419103232000000 * T * T - 0.03246626956972881250) * G;
    newValues.B = -(184467.44073709551616000000 * T * T -
                    8116.93632731367731603232 * T + 0.01623313478486440625) *
                  G;
    newValues.C = (184467.44073709551616000000 * T * T +
                   16233.50371974588044103232 * T + 0.01623313478486440625) *
                  G;
    newValues.D =
        (368934.88147419103232000000 * T * T - 0.03246626956972881250) * G;
    newValues.E = (184467.44073709551616000000 * T * T -
                   16233.50371974588044103232 * T + 0.01623313478486440625) *
                  G;
    ...
    G = 1 / (976585388.65744263019233280000 * T +
             4296.97571009274828800000 * V1 + 4296.97571009274828800000 * V2 +
             47223664828.69645213696000000000 * T * T +
             4722366.48286964521369600000 * T * T * V1 +
             4722366.48286964521369600000 * T * T * V2 +
             305442.66411200865242185728 * T * V1 +
             326221.07663663509136211968 * T * V2 +
             0.09142501510835634375 * V1 * V2 +
             2.07784125246264400000 * T * V1 * V2);
    newValues.I =
        -(-8593.95142018549657600000 * V1 - 8593.95142018549657600000 * V2 +
          94447329657.39290427392000000000 * T * T -
          0.18285003021671268750 * V1 * V2 +
          9444732.96573929042739200000 * T * T * V1 +
          9444732.96573929042739200000 * T * T * V2) *
        G;
    newValues.J =
        -(4296.97571009274828800000 * V1 - 976585388.65744263019233280000 * T +
          4296.97571009274828800000 * V2 -
          305442.66411200865242185728 * T * V1 -
          326221.07663663509136211968 * T * V2 +
          0.09142501510835634375 * V1 * V2 +
          4722366.48286964521369600000 * T * T * V1 +
